...
def program(conn,url):
  ...
  with open("/www/head.html",'r') as f:
    html=f.read().format("List program")
...
def car_remote(conn,url,port,k):
  print("car_remote")
  with open("/www/car_remote.html",'r') as f:
    send_response(conn,str(f.read()).replace("{0}",hotspot.ifconfig()[0]).replace("{1}",str(port)).replace("{2}",k))
