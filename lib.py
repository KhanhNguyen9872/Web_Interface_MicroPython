...
def process(conn,ssid,password):
  if str(wifi.isconnected())=="True":
    ...
    if int(count)==int(timeout):
      ...
      Pin(2,Pin.IN)
      print("TIMEOUT {0}s: {1}".format(str(timeout),str(ssid)))
      with open("/www/cannot_connect.html",'r') as f:
        response=str(f.read().format(str(ssid),str(timeout)))
    else:
      Pin(2,Pin.OUT)
      print("SUCCESS: {0}\nIP: {1}\nSubnet: {2}\nGateway: {3}\nDNS: {4}".format(ssid,str(wifi.ifconfig()[0]),str(wifi.ifconfig()[1]),str(wifi.ifconfig()[2]),str(wifi.ifconfig()[3])))
      write_file(ssid,password)
      with open("/www/connected.html",'r') as f:
        response=str(f.read().format(str(ssid),str(wifi.ifconfig()[0]),str(wifi.ifconfig()[1]),str(wifi.ifconfig()[2]),str(wifi.ifconfig()[3]),str("/"),str("Home")))
